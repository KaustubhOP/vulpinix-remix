...
const jsx = `
  // Added logic to fix bugs

  const isPlatformSelected = platforms.some(p => p.enabled);
  const steps = [
    { id: 1, label: 'Upload File', completed: !!uploadedFile },
    { id: 2, label: 'AI Captions', completed: !!uploadedFile && !!aiAnalysis.caption },
    { id: 3, label: 'Choose Platforms', completed: !!uploadedFile && isPlatformSelected },
    { id: 4, label: 'Schedule', completed: !!uploadedFile && !!scheduleDate },
  ...
  return (
    <motion.div 
      ...
      style={{
        background: '#080b14',
        backgroundImage: 'radial-gradient(ellipse 60% 50% at 20% 10%, rgba(99,51,255,0.15) 0%, transparent 60%), radial-gradient(ellipse 50% 40% at 80% 90%, rgba(0,212,200,0.1) 0%, transparent 60%), linear-gradient(rgba(99,51,255,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(99,51,255,0.06) 1px, transparent 1px)',
    ...
    >
      ...
      <motion.div 
        ...
        className="fixed top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-[#6333ff] to-[#06d6c7] z-[100]" 
      />

      <div className="flex flex-col lg:flex-row max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16 relative z-10 gap-x-16 gap-y-12">
        {/* Step Indicator (Desktop) */}
        <div className="hidden lg:block w-72 flex-shrink-0 sticky top-24 h-fit self-start mt-6">
          <div className="relative pt-4 pl-4 pb-8">
            {/* Background Line */}
            <div className="absolute left-[27px] top-8 bottom-12 w-[2px] bg-gray-800" />
            
            {/* Active Gradient Line */}
            <motion.div 
              className="absolute left-[27px] top-8 w-[2px] bg-gradient-to-b from-[#6333ff] to-[#06d6c7]"
              initial={{ height: '0%' }}
              animate={{ height: \`\${(Math.min(currentStepIndex, 4) / 4) * 100}%\` }}
            ...
            <div className="space-y-12 relative z-10">
              {steps.map((step, idx) => {
                ...
                return (
                  <div key={step.id} className="flex items-center gap-5 transition-all duration-300">
                    <div className={\`w-7 h-7 rounded-full flex items-center justify-center transition-all duration-500 \${isCompleted ? 'bg-[#06d6c7] shadow-[0_0_15px_rgba(6,214,199,0.5)]' : isActive ? 'bg-[#080b14] border-2 border-[#6333ff] shadow-[0_0_15px_rgba(99,51,255,0.5)]' : 'bg-[#080b14] border-2 border-gray-700'}\`}>
                      {isCompleted && <Check className="w-4 h-4 text-black font-bold" />}
                    </div>
                    <span className={\`text-sm tracking-wide transition-all \${isCompleted ? 'text-[#06d6c7]' : isActive ? 'text-white font-medium scale-105 origin-left' : 'text-gray-500'}\`}>
        ...
        <div className="flex-1 max-w-4xl min-w-0">
          ...
          <div className="mb-16 pt-2">
            ...
            <h1 className="text-4xl sm:text-5xl lg:text-6xl mb-6 font-['Syne'] font-[800] leading-tight mt-4">
            ...
            >
              {!uploadedFile ? (
                <div
                  ...
                  className={\`relative rounded-[16px] p-12 text-center transition-all duration-300 cursor-pointer group \${
                    dragActive
                      ...
                      : "border-[2px] border-dashed border-[rgba(99,51,255,0.4)] bg-[rgba(99,51,255,0.03)]"
                  } hover:border-[#6333ff] hover:bg-[rgba(99,51,255,0.08)] hover:scale-[1.01]\`}
                >
                  {/* Decorative rotating border gradient layer for drag and drop */}
                  <motion.div 
                     animate={{ rotate: 360 }} 
                     transition={{ repeat: Infinity, duration: 10, ease: "linear" }} 
                     className="absolute inset-[-5px] rounded-[18px] opacity-0 group-hover:opacity-20 bg-[conic-gradient(from_0deg,transparent,rgba(99,51,255,0.8),rgba(6,214,199,0.8),transparent)] -z-10 transition-opacity duration-500" 
                  />
                  ...
                  <div className="flex flex-col items-center justify-center pointer-events-none">
                    <motion.div 
                      ...
                      className="w-20 h-20 rounded-full bg-gradient-to-tr from-[#6333ff] to-[#06d6c7] p-[2px] mb-8 shadow-xl shadow-[rgba(6,214,199,0.2)] mx-auto flex shrink-0 items-center justify-center"
                    >
                      <div className="w-full h-full bg-[#080b14] rounded-full flex items-center justify-center">
                        <Upload className="w-8 h-8 text-white transition-transform duration-200 group-hover:scale-110 group-hover:text-[#06d6c7]" />
                      </div>
                    </motion.div>
                    
                    <h3 className="text-2xl font-['Syne'] font-[600] text-white mb-3">
                ...
                <motion.div 
                  ...
                  className="flex flex-col sm:flex-row items-center gap-6 p-6 rounded-[16px] bg-gradient-to-br from-[rgba(99,51,255,0.1)] to-[rgba(0,212,200,0.05)] border border-[rgba(255,255,255,0.08)] relative overflow-hidden"
                >
                  <div className="w-24 h-24 rounded-[12px] overflow-hidden border border-[rgba(255,255,255,0.1)] shadow-xl shrink-0 bg-[#080b14] flex items-center justify-center relative z-10">
                  ...
                  <div className="flex-1 w-full flex flex-col justify-center relative z-10">
                    <p className="text-white font-['Syne'] font-[600] text-[16px] truncate mb-2 pr-8">{uploadedFile.file.name}</p>
                    <p className="text-sm text-[rgba(180,180,220,0.5)] font-medium mb-4">{(uploadedFile.file.size / 1024 / 1024).toFixed(2)} MB • {progress < 100 ? 'Uploading...' : 'Completed'}</p>
                  ...
                  <button
                    ...
                    className="absolute top-4 right-4 text-gray-500 hover:text-white p-2 rounded-full transition-all duration-300 hover:bg-[rgba(239,68,68,0.2)] hover:shadow-[0_0_15px_rgba(239,68,68,0.4)] hover:text-red-400 group z-20"
                  >
              ...
              {generatingCaption && (
                <div className="py-12 space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="w-full h-4 bg-gray-800 rounded-full relative overflow-hidden">
                       <motion.div animate={{ x: ["-100%", "200%"] }} transition={{ repeat: Infinity, duration: 1.5 }} className="absolute inset-y-0 left-0 w-1/2 bg-gradient-to-r from-transparent via-[rgba(99,51,255,0.3)] to-transparent" />
                    </div>
                    <div className="w-5/6 h-4 bg-gray-800 rounded-full relative overflow-hidden">
                       <motion.div animate={{ x: ["-100%", "200%"] }} transition={{ repeat: Infinity, duration: 1.5, delay: 0.1 }} className="absolute inset-y-0 left-0 w-1/2 bg-gradient-to-r from-transparent via-[rgba(99,51,255,0.3)] to-transparent" />
                    </div>
                    <div className="w-3/4 h-4 bg-gray-800 rounded-full relative overflow-hidden">
                       <motion.div animate={{ x: ["-100%", "200%"] }} transition={{ repeat: Infinity, duration: 1.5, delay: 0.2 }} className="absolute inset-y-0 left-0 w-1/2 bg-gradient-to-r from-transparent via-[rgba(99,51,255,0.3)] to-transparent" />
                    </div>
                  </div>
                </div>
              )}
                      ...
                      {editingHashtags ? (
                        <div className="space-y-4">
                          ...
                          <div className="flex gap-2">
                            ...
                            />
                            <Button onClick={addHashtag} type="button" className="bg-[rgba(6,214,199,0.1)] text-[#06d6c7] hover:bg-[#06d6c7] hover:text-white transition-colors">Add</Button>
                          </div>
                        </div>
                      ) : (
                     ...
                     {previewImage ? (
                       <div className="space-y-4">
                         <div className="relative rounded-lg overflow-hidden border border-[rgba(255,255,255,0.1)] group/img h-48">
                           ...
                           <button onClick={(e) => { e.stopPropagation(); setPreviewImage(null); localStorage.removeItem('adPreviewImage'); }} className="absolute top-2 right-2 bg-red-500/80 hover:bg-red-500 backdrop-blur text-white p-1.5 rounded-md transition-colors"><X className="w-4 h-4" /></button>
                         </div>
                       </div>
                     ) : (
...
console.log('Fixed file layout issues.');
